...
describe("App", () => {
  ...
  it("shows compact grouped results after the final vote", () => {
    ...
    const popover = screen.getByRole("tooltip", { name: "Artwork details" })

    expect(popover).toBeInTheDocument()
    expect(within(popover).getByRole("img", { name: "Selected artwork" })).toBeInTheDocument()
    expect(within(popover).queryByText("Girl In Field")).not.toBeInTheDocument()
    expect(within(popover).queryByText("Item 4")).not.toBeInTheDocument()
    expect(within(popover).queryByText("Your answer: Human")).not.toBeInTheDocument()
    expect(within(popover).queryByText("Correct answer: AI")).not.toBeInTheDocument()
    expect(
      within(popover).getByText(/🤖 This image was generated by Ryan Wise/),
    ).toBeInTheDocument()
  })
})
