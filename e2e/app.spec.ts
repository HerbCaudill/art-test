...
test("completes a full attempt, locks results, reviews details, and starts over", async ({
  ...
  const popover = page.getByRole("tooltip", { name: "Artwork details" })
  await expect(popover).toBeVisible()
  await expect(popover.getByRole("img", { name: "Selected artwork" })).toBeVisible()
  await expect(popover.getByText("Girl In Field")).toHaveCount(0)
  await expect(popover.getByText(/🤖 This image was generated by Ryan Wise/)).toBeVisible()
  await expect(popover.getByText("Correct answer: AI")).toHaveCount(0)
...
test("opens answer-key details with keyboard focus", async ({ page }) => {
  ...
  await expect(page.getByRole("tooltip", { name: "Artwork details" })).toBeVisible()
})
